...
export class StartCommand implements IContainerCommand {
    ...
    async execute(container: Docker.Container) { await container.start(); }
}

export class StopCommand implements IContainerCommand {
    get name() { return 'stop'; }
    async execute(container: Docker.Container) { await container.stop(); }
}

export class RestartCommand implements IContainerCommand {
    get name() { return 'restart'; }
    async execute(container: Docker.Container) { await container.restart(); }
}
